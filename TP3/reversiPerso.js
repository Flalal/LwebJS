...
function verifSetAttributDe(i){
    if(verifColVersHaut(i)) {
            var j = i-dimension;
            while(tabBoutons[j].getAttribute('class') != (aQuiLeTour % 2 == 0 ? BOUTON_JOUEUR1 : BOUTON_JOUEUR2)){
                tabBoutons[j].setAttribute('class', (aQuiLeTour % 2 == 0 ? BOUTON_JOUEUR1 : BOUTON_JOUEUR2));
                j-=dimension;
            }
        }
...
function justeVerif(i){
    return verifColVersBas(i) || verifColVersHaut(i) || verifDiagoVersBasDroite(i) || verifDiagoVersBasGauche(i) || verifDiagoVersHautDroite(i) || verifDiagoVersHautGauche(i) || verifRawVersDroite(i) || verifRawVersGauche(i);
}
function verifColVersHaut(i) {
     i -= dimension;
     if (i <= 0)
         return false;
     if (tabBoutons[i].getAttribute('class') == (aQuiLeTour % 2 == 0 ? BOUTON_JOUEUR1 : BOUTON_JOUEUR2)) {
         return true;
     }
